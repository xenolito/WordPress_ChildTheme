...
/*------------------------------------------------------------------------------------------------------*
						ADD PRELOAD TO LINK REL FONTS added by wp_enqueue_style with handle starting with 'pct-font-'
\*------------------------------------------------------------------------------------------------------*/
// AÑADIMOS PRELOAD para la fuentes
add_filter('style_loader_tag', 'my_style_loader_tag_filter',10,2);

function my_style_loader_tag_filter($html, $handle) {
	if(strpos($handle, 'pct-font-') !== false) {
