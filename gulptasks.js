...
const webpackConfig = process.env.NODE_ENV === 'development' ? './webpack.config.js' : './webpack.config.prod.js'
...
const jsSourceOrder = [baseSourcePath + 'js/*.js', baseSourcePath + 'js/custom_theme_scripts.js']

function loadConfigData(done) {
    configData = require('./configSetup.json')
    console.log('configData cargado correctamente, localhost===> ' + configData.localhost)
    done()
}
...
function checkifWPfolderExists(str) {
    ...
    try {
        valid = fs.existsSync(httpdocsPath + str) ? true : 'El directorio no existe'
...
function checkChildFolder(str) {
    let valid
    const httpdocsPath = setupOBJ.httpdocs_path
    const pathToCheck = setupOBJ.httpdocs_path + setupOBJ.wp_folder + '/wp-content/themes/' + str

    try {
        valid = fs.existsSync(pathToCheck) ? true : 'El directorio del tema hijo NO EXISTE'
    } catch (err) {
        console.log('ERROR')
    }
    return valid
...
function setUserValues(values) {
    ...
    setupOBJ.destChildTheme = setupOBJ.destWP + 'wp-content/themes/' + setupOBJ.childThemeFolder + '/'
    ...
    return file('configSetup.json', JSON.stringify(setupOBJ), { src: true }).pipe(dest('./'))
}
...
function directories(done) {
    ...
    src('mu-plugins/*.php', { base: '.' }).pipe(dest(setupOBJ.destChildTheme + '../../')) // 2: copy mu-plugins/ needed for PICTAU's WPO.
    ...
    console.log('Path to wp-config.php ==> ' + setupOBJ.httpdocs_path + setupOBJ.wp_folder)
        ...
        .pipe(
            breplace({
                wpmailfrom_name: {
                    ...
                    tpl: "function custom_wp_mail_from_name( $original_email_from ) {return '%s';}",
                },
                wpmailfrom_email: {
                    src: setupOBJ.wp_mailfrom_email,
                    tpl: "function custom_wp_email_address( $original_email_address ) { return '%s'; }",
                },
            })
        )
        .pipe(dest('./'))
...
function scssTask() {
    ...
    return src(cssSourceOrder)
        ...
        .pipe(postcss([autoprefixer('last 2 versions'), cssnano({ preset: ['default', { discardComments: true }] })])) // PostCSS plugins
...
function browSync(done) {
    console.log('LOCALHOST FOR BROWSERSYNC on configData.localhost= ' + configData.localhost)
    console.log('LOCALHOST FOR BROWSERSYNC on setupOBJ.localhost= ' + setupOBJ.localhost)
    bsync.init({
        injectChanges: true,
        watch: true,
        proxy: {
